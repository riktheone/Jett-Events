...
class EventoDAO(BaseDAO):
    ...
    def obterTodosPorPeriodo(self, dtInicio, dtFim):
        ...
        return self.obterRegistrosPorParametros(
            """
            select id, nome, descricao, dt_inicio, dt_fim, dt_limite_inscricao,
                   vagas_disponiveis, cpf_responsavel, nome_responsavel, email_responsavel
            from eventos
    ...
    def atualizar(self, id, nome, descricao, dtInicio, dtFim, dtLimiteInscricao, vagasDisponiveis, cpfResponsavel, nomeResponsavel, emailResponsavel):
        ...
        return self.executarComandoDML(
            """
            update eventos
            set nome = ?, descricao = ?, dt_inicio = ?, dt_fim = ?, dt_limite_inscricao = ?,
                vagas_disponiveis = ?, cpf_responsavel = ?, nome_responsavel = ?, email_responsavel = ?
            where id = ?
            """,
            parametros,
    ...
    def inscrever(self, id_evento, id_usuario_participante, participante):
        parametros = [
            id_evento,
        ...
        return self.executarComandoDML(
            ...
            insert into inscricao_evento (
                id_evento, id_usuario, dt_inscricao, cpf_participante,
    ...
    def removerInscricao(self, id_evento, id_participante):
        parametros = [id_evento, id_participante]
        return self.executarComandoDML(
            "delete from inscricao_evento where id_evento = ? and id_usuario = ?",
            parametros,
        )

...
export class PalestraForm implements OnInit {
  ...
  nome = ''; descricao = '';
  dt_palestra = '';
  horario_inicio_palestra = ''; horario_fim_palestra = '';
  ...
  private preencher(p: Palestra): void {
    this.nome = p.nome; this.descricao = p.descricao ?? '';
    this.dt_palestra = p.dt_palestra;
    this.horario_inicio_palestra = p.horario_inicio_palestra;
    this.horario_fim_palestra = p.horario_fim_palestra;
  ...
  async salvar(): Promise<void> {
    if (!this.nome || !this.nome_palestrante || !this.dt_palestra ||
        !this.horario_inicio_palestra || !this.horario_fim_palestra) {
      this.erro.set('Preencha título, palestrante, data e horários.'); return;
    }
    const payload = {
